...
def get_verified_clusters():
    """Fetch, cluster, and return only cross-verified groups (2+
    independently owned outlets -- see SOURCE_GROUPS) as a list of dicts
    shaped for the rest of the pipeline: {items, sources, source_count}."""
    items = fetch_items()
    ...
    for group in groups:
        ...
        if len(owner_groups) >= MIN_SOURCES_FOR_VERIFIED:
            verified.append({
                "items": group,
                "sources": sources,
    ...
    verified.sort(key=lambda c: c["source_count"], reverse=True)
    return verified


def main():
    print(f"Fetching from {len(FEEDS)} feeds...\n")
    verified = get_verified_clusters()
    print("=" * 78)
    print(f"CROSS-VERIFIED STORIES ({len(verified)} clusters, "
          f"{MIN_SOURCES_FOR_VERIFIED}+ independent outlets each)")
    print("=" * 78)
    for c in verified:
        print(f"\n[{c['source_count']} sources: {', '.join(c['sources'])}]")
        for it in c["items"]:
