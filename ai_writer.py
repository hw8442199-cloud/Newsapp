...
# SOURCE_PROFILES: factual outlet descriptors (ownership/format, not a
# trust score) -- fed into the prompt below so the reliability_note the
# model writes is grounded in given facts, not invented. Try the package
# import first; fall back to a same-directory import so this file still
# runs standalone (python pipeline/ai_writer.py) as it did before.
try:
    from .cluster_news import SOURCE_PROFILES
except ImportError:
    from cluster_news import SOURCE_PROFILES
...
headlines and summaries from one or more outlets reporting on the same \
real-world story, each with a factual profile (ownership/format -- not a \
trust judgment), plus whether the story is cross-verified (2+ \
independently owned outlets) or single-source. Synthesize them into one \
original, neutral, well-structured news article — not a copy or close \
paraphrase of any single source.
...
- Attribution depends on sourcing status. If CROSS-VERIFIED, you may \
state facts plainly, as corroborated news. If SINGLE-SOURCE, attribute \
claims throughout to the reporting outlet ("X reports that...", \
"according to X...") rather than stating them as flatly established \
fact — the reader needs to know this is one outlet's reporting, not yet \
confirmed by anyone else.
- reliability_note: write one neutral sentence for the reader about \
sourcing confidence, grounded ONLY in the outlet profile(s) and sourcing \
status you were given (ownership, whether the outlet does original \
reporting or aggregates others', and whether the story is corroborated). \
Never invent a trust score, bias rating, or claim you weren't given.
...
def _build_user_prompt(cluster):
    ...
    status = (
        f"CROSS-VERIFIED ({cluster['source_count']} independently owned outlets)"
        if cluster.get("verified")
        else "SINGLE-SOURCE (not yet corroborated by another outlet)"
    )
    lines = [f"Sourcing status: {status}\n", "Source reports on this story:\n"]
    for item in deduped:
        profile = SOURCE_PROFILES.get(item["source"], "profile unknown")
        lines.append(f"— {item['source']} [{profile}]: {item['title']}")
...
def _valid(parsed):
    if not isinstance(parsed, dict):
        return False
    for key in ("headline", "dek", "body", "reliability_note"):
...
def write_article(cluster):
    ...
    return {k: parsed[k].strip() for k in ("headline", "dek", "body", "reliability_note")}
